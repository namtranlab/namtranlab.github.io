...
A martingale is the mathematical model of a <em>fair game</em>. At every moment, no matter what has happened so far, the expected future value of the process equals its current value. The concept is built directly on conditional expectation from §1.1 — a martingale is precisely a process whose conditional expectation at any future time equals its present value.
</div>
...
<div class="key-idea"><strong>A martingale has constant expected value.</strong> Taking the full expectation: $E[M_n] = E[E[M_n \mid \mathcal{F}_0]] = E[M_0]$ for all $n$. The mean is time-invariant — a necessary (but not sufficient) condition for fairness.</div>
...
<div class="misconception-block">
  ...
  <div class="mc-wrong"><strong>Wrong:</strong> "If $E[M_n] = E[M_0]$ for all $n$, then $M_n$ is a martingale."</div>
  <div class="mc-correct"><strong>Correct:</strong> Constant unconditional mean is a <em>necessary</em> but not <em>sufficient</em> condition. The martingale property is the stronger statement $E[M_n \mid \mathcal{F}_m] = M_m$ — a condition on the full conditional distribution given every possible history, not just on the average over all histories.</div>
</div>
...
### Part 7 — Connection to §1.1 Properties
...
<table>
  <thead>
    <tr>
      <th>Martingale argument</th>
      <th>§1.1 property used</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>$E[S_n \mid \mathcal{F}_n] = S_n$ — current sum is known</td>
      <td>Property 1 — Known $Y$</td>
    </tr>
    <tr>
      <td>$E[X_{n+1} \mid \mathcal{F}_n] = E[X_{n+1}] = 0$ — future increment is independent</td>
      <td>Property 3 — Independence</td>
    </tr>
    <tr>
      <td>$E[S_n X_{n+1} \mid \mathcal{F}_n] = S_n E[X_{n+1} \mid \mathcal{F}_n]$ — $S_n$ pulls out</td>
      <td>Property 5 — Constants rule</td>
    </tr>
    <tr>
      <td>$E[W_n \mid \mathcal{F}_n] = W_n$ — current winnings are known</td>
      <td>Property 1 — Known $Y$</td>
    </tr>
    <tr>
      <td>$E[B_{n+1}(M_{n+1} - M_n) \mid \mathcal{F}_n] = B_{n+1} \cdot 0$ — $B_{n+1}$ pulls out</td>
      <td>Property 5 — Constants rule</td>
    </tr>
    <tr>
      <td>One-step check implies all-future check</td>
      <td>Property 2 — Tower property</td>
    </tr>
  </tbody>
</table>
...
- A **martingale** $M_n$ with respect to $\{\mathcal{F}_n\}$ satisfies $E[M_n \mid \mathcal{F}_m] = M_m$ for all $m < n$ — the current value is the best predictor of all future values. It models a fair game with no predictable drift.
- **One-step criterion:** it suffices to verify $E[M_{n+1} \mid \mathcal{F}_n] = M_n$ for every $n$; the tower property propagates this to all future times. Constant expected value $E[M_n] = E[M_0]$ follows as an immediate corollary.
- **Sub/supermartingales:** replace $=$ with $\geq$ (submartingale, favourable game) or $\leq$ (supermartingale, unfavourable game). The naming is counterintuitive — a supermartingale's expected value *decreases*. Every martingale is both.
- **Example 1.2.1:** $S_n = X_1 + \cdots + X_n$ with $E[X_j] = 0$ is a martingale. Verified in one step using Properties 1 and 3 from §1.1.
- **Example 1.2.2:** $M_n = S_n^2 - A_n$ where $A_n = \sum_{j=1}^n \sigma_j^2$ is a martingale. $S_n^2$ is a submartingale; subtracting the cumulative variance (the predictable compensator) restores fairness. This is the discrete quadratic variation.
- **Discrete stochastic integral:** $W_n = \sum_{j=1}^n B_j \Delta M_j$ is a martingale when $B_j$ is predictable ($\mathcal{F}_{n-1}$-measurable) and bounded. No allowable betting strategy can systematically beat a fair game in finite time. This is the discrete prototype of the Itô integral.
- **Martingale betting strategy (Example 1.2.4):** shows that in *infinite* time, the game can be beaten — but only by risking exponentially growing losses. The finite-time martingale property of $W_n$ is preserved; the "winning" only emerges in the limit, motivating the Optional Sampling Theorem in §1.3.

<div class="ref-tags">
<span class="ref-tag">Martingale</span>
<span class="ref-tag">Submartingale</span>
<span class="ref-tag">Supermartingale</span>
...
<span class="ref-tag">Predictable compensator</span>
